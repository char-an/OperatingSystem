...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < 1000; i++)
    {
        ...
        // using .join(), we will wait for previous iteration to finish
        t1.join();
        t2.join();
        t3.join();

        // now, we detach threads from the main thread, so that main thread can make progress without waiting for the threads to finish.
        t1 = thread(thread_s1, input_image, &smoothened_image);
        t2 = thread(thread_s2, input_image, &smoothened_image, &details_image);
        t3 = thread(thread_s3, input_image, &details_image, &sharpened_image);
    }

    // now for the last iteration, we again use .join() so that main thread waits for them.
    t1.join();
    t2.join();
    t3.join();
